...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, output1, output2, label):
        ...
        norm = torch.norm(output1-output2, dim=1)
        m_norm = m - norm
        m_norm = torch.clamp(m_norm, 0)
        loss = (1-label) * norm**2 + label * m_norm**2
        loss = loss.mean()
        return loss


def evaluate(dataiter, net, split, device):
    similarity = np.zeros((2, 10))
    for i in range(2):
...
if __name__ == "__main__":
    ...
    train_similarity = evaluate(dataiter, net, 'train', device)
 """
    ...
    test_similarity = evaluate(dataiter, net, 'test', device)    

    plt.plot(range(len(loss_history)), loss_history)
    plt.savefig('att_test/loss.png')
