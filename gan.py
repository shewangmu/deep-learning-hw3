...
def discriminator_loss(logits_real, logits_fake, dtype):
    ...
    ######### TO DO ###########
    ###########################
    real_loss = bce_loss(logits_real, torch.ones_like(logits_real))
    fake_loss = bce_loss(logits_fake, torch.zeros_like(logits_fake))
...
def run_a_gan(D, G, D_solver, G_solver, discriminator_loss, generator_loss,
              loader_train, dtype, show_every=50, batch_size=128, noise_size=96,
              num_epochs=10):
    """
    Train a GAN!
    ...
    for epoch in range(num_epochs):
        for x, _ in loader_train:
            ...
            if (iter_count % show_every == 0):
                ...
                plt.pause(1.0)
                print()
                print('elapse time: {}'.format(time.time()-start))
                start = time.time()
            iter_count += 1
